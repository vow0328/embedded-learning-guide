...
如果任务函数真的返回了，内核会认为这是编程错误——返回意味着这个任务"结束"了，但任务没有"结束"这个状态。FreeRTOS 对返回之后的行为没有定义，也不会替你把任务清理掉；要结束一个任务，必须显式调用删除任务的接口。所以**任何写在循环外面的收尾代码都不会被执行**，这是新手常见的困惑。
...
FreeRTOS 里每个任务有独立的栈，大小在创建任务时指定。**它不是自动增长的**，给多少就是多少。栈里放的是局部变量、函数调用的返回地址、中断嵌套时的现场。栈给少了会溢出，溢出的后果见后面的"任务栈该给多少"和 4.4。
...
关于资料，有个现实得先讲明白：**FreeRTOS 的中文资料大多还停在"手动移植"的年代**——去官网下载源码、把 Source 目录拷进工程、手动配头文件路径、改 FreeRTOSConfig.h，最后还要去启动文件里改几个中断入口的名字。这套步骤不是 RTOS 的知识，是十几年前没有图形化工具时的产物；今天 CubeMX 勾一下就能生成，不值得在这上面耗时间。

**分辨一套资料还值不值得看，有个很省事的判断：看它前半小时在讲什么。** 教你"怎么把源码加进工程"的，是过时的路子；讲"任务为什么会饿死""队列为什么没阻塞住"的，才是你要的。

真正讲得系统的还是韦东山的 [FreeRTOS 入门与工程实践 —— 由浅入深带你学习 FreeRTOS](https://www.bilibili.com/video/BV1Jw411i7Fz)（12 小时多，基于 STM32、以实际项目为导向，也是少有的不再手动移植源码的教程）。它对刚从裸机转过来的人偏难，节奏快、信息密度大，容易一上来就被劝退——**所以别从它开始，先用上面那五步把任务、队列、信号量跑通**。那五步本身就是最轻的入门路径，不必再去找一套"轻量教程"；等你能点灯、能在两个任务间传数据，再回来看韦东山这套，吸收效率会高很多。

还有一件动手前该分清的事：**CubeMX 生成的 FreeRTOS 工程走的是 CMSIS-RTOS 封装层**（CubeMX 里 FreeRTOS 的 Interface 一项，可选 CMSIS v1 或 v2，两个都是封装层，都跟原生 API 名字对不上）。你在图形界面里加的任务，生成出来是 `osThreadNew()`，而不是 FreeRTOS 原生的 `xTaskCreate()`；延时是 `osDelay()` 而不是 `vTaskDelay()`；队列是 `osMessageQueueNew()` 而不是 `xQueueCreate()`。两套 API 说的是同一件事，底层也是同一个内核——`osThreadNew()` 内部仍然调用 `xTaskCreate()`——但名字和参数都不一样。

| 做的事 | 原生 API（教程、教材、面试题里常见） | CubeMX 默认生成 |
|---|---|---|
| 创建任务 | `xTaskCreate()` | `osThreadNew()` |
| 延时 | `vTaskDelay()` | `osDelay()` |
| 创建队列 | `xQueueCreate()` | `osMessageQueueNew()` |
| 创建互斥量 | `xSemaphoreCreateMutex()` | `osMutexNew()` |
| 启动调度器 | `vTaskStartScheduler()` | `osKernelStart()` |

于是你会撞上一种很打击人的情况：**教程里敲得好好的代码，贴进自己的工程编译不过。** 不是你理解错了，是你们站在封装层的两侧。

对策是：**先认清自己手上的工程属于哪一层，再决定跟哪套代码对齐，然后一门跟到底。** 最要避免的是两套混着抄——比如把 CMSIS 的优先级枚举 `osPriorityNormal` 直接塞给 `xTaskCreate()` 的优先级参数，能编译过，但两个数值范围根本不是一回事，调度行为会变得莫名其妙。好在原理性的内容（调度、阻塞、优先级反转）两套完全共通，切换时受影响的只有函数名。
