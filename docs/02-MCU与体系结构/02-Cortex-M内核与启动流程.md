...
**先分清两个层级：内核是"核"，架构是这套核遵循的指令集规范。** ARM 设计的是内核（Cortex-M3、Cortex-M4、Cortex-M7 这些），每个内核又实现某一代架构（ARMv6-M、ARMv7-M、ARMv7E-M、ARMv8-M）。一颗 STM32F407 拆开是三层：ST 的外设和存储器 + Cortex-M4 内核 + ARMv7E-M 架构。**所以"Cortex-M4 架构"这个说法是不准确的，Cortex-M4 是内核，它实现的架构叫 ARMv7E-M。**

分清它有什么用？因为**"这颗芯片是哪个内核"决定你能用哪些指令、工具链要配哪个选项、有没有 TrustZone**，而架构正是这些差异的来源：

- **M0/M0+ 用不了 FPU 和 DSP 指令**，这不是厂商省成本，是 ARMv6-M 里根本没有这两样东西；M4/M7 有，是 ARMv7E-M 定义进去的。
- **编译器的 `-mcpu` / `-march` 必须和芯片对上**（`-mcpu=cortex-m4` 对应 `thumbv7em`）。选错了不会提示"芯片不匹配"，只会在某条指令上给你一个看不懂的汇编错误。
- **TrustZone 只有 M33 及以后才有**，它是 ARMv8-M 才引入的；在 ARMv7-M 的内核上做安全启动，只能靠外部手段。
- **CMSIS 里有些函数在 M0 上是软件模拟的**（比如前导零计数 `__CLZ`、除法），因为 ARMv6-M 缺少对应的单条指令。

所以下面这张表把架构单独列了一列——**横向比内核时，真正决定"能不能用某个特性"的是架构那一列。**

| 内核 | 架构 | 典型型号 | 关键特征 | 定位 |
|---|---|---|---|---|
| **Cortex-M0** | ARMv6-M | STM32F030 | 总线结构最简单、指令集最小、无硬件除法 | 极低成本场景，替代 8 位机 |
| **Cortex-M0+** | ARMv6-M | STM32G0、LPC800 | 在 M0 基础上加总线优化、单周期 IO | 低功耗、低成本，比 M0 更值得选 |
| **Cortex-M3** | ARMv7-M | STM32F103 | 指令与数据总线分开、硬件除法、位带、无 FPU | 最经典的入门内核，教程最多 |
| **Cortex-M4** | ARMv7E-M | STM32F407、GD32F4 | M3 全部特性 + 单精度 FPU + DSP 指令 | 电机控制、音频、信号处理的主力 |
| **Cortex-M7** | ARMv7E-M | STM32F7/H7 | 双发射 6 级流水、I-Cache/D-Cache、TCM、双精度 FPU | 高性能计算，跑图形和复杂算法 |
| **Cortex-M33** | ARMv8-M | STM32L5/U5、GD32E5 | M4 特性 + TrustZone 安全隔离 | 安全物联网、金融终端 |
| **Cortex-M55** | ARMv8.1-M | 部分 AI 芯片 | M 系列首个带 Helium 向量单元（MVE） | 边缘 AI、机器学习推理 |
...
| 0xE000_0000 ~ 0xFFFF_FFFF | 系统区 | 内核外设都在这里，统称 PPB（Private Peripheral Bus，私有外设总线），实际用到 0xE00F_FFFF 为止：NVIC、SysTick、SCB、MPU、ITM |
...
| 6 | UsageFault | 用法错误，比如除零（要先打开 CCR 的 DIV_0_TRP 位，见 [3.6 调试方法论](../03-嵌入式软件工程/06-调试方法论.md)）、非对齐访问、执行了非法指令 |
| 11 | SVCall | 由 SVC 指令触发，RTOS 的系统调用入口 |
| 14 | PendSV | 可挂起的系统异常，RTOS 用它做任务切换 |
| 15 | SysTick | 系统节拍定时器，内核外设，不是外设中断 |
...
SysTick 是一个 24 位的递减计数器，属于**内核外设（core peripheral）**：它挂在私有外设总线（PPB）上，寄存器地址固定在 0xE000_E010，由 ARM 定义而不是由厂商定义——这和你常用的 TIM 那种落在片上外设区（0x4000_0000 起）的外设不是一类东西。这个身份带来几个别的定时器没有的优点：

1. **所有 Cortex-M 都有**，寄存器地址固定，代码可以跨厂商复用——这正是因为它算内核的一部分，不算某家厂商的外设。
2. **不需要开 RCC 时钟**。厂商的片上外设复位后时钟默认关闭，用之前必须先开；SysTick 跟内核共用时钟，没有这一步。
3. **异常号固定是 15**，属于系统异常，不用查数据手册，也不会出现在厂商的 IRQ 编号里。

顺带记住这个分界的用处：**内核外设看 ARM 的架构手册，片上外设看厂商的参考手册。** 遇到"这个寄存器该查哪本书"的困惑时，先问它是不是内核外设。
...
1. **整篇不用先读，但有两节要先看。** 按本章 README 的建议，先把 2.3 通用外设跑一遍、手上有点感觉了再回来啃这一篇。例外是《异常与中断模型》和《NVIC：真正干活的中断控制器》两节——[2.4 中断系统与低功耗](04-中断系统与低功耗.md) 整篇建立在这两节上，要在读它之前先看完。
