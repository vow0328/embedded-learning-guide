...
> **前置知识**：[1.2 编译链接与内存布局](../01-编程与软件基础/02-编译链接与内存布局.md) · [2.1 单片机平台选型](01-单片机平台选型.md)
> **掌握标准**：能说清 Cortex-M 内核的寄存器组、特权级别、异常模型，并能独立分析一次上电启动到 main 的完整过程
...
| 内核 | 典型型号 | 关键特征 | 定位 |
|---|---|---|---|
| **Cortex-M0** | STM32F030 | 冯诺依曼总线、指令集最小、无硬件除法 | 极低成本场景，替代 8 位机 |
| **Cortex-M0+** | STM32G0、LPC800 | 在 M0 基础上加总线优化、单周期 IO | 低功耗、低成本，比 M0 更值得选 |
| **Cortex-M3** | STM32F103 | 哈弗总线、硬件除法、位带、无 FPU | 最经典的入门内核，教程最多 |
...
## 寄存器组：16 个通用寄存器加一个状态寄存器

Cortex-M 的寄存器模型非常清爽，一共只有 16 个通用寄存器和 1 个状态寄存器。这个数量是刻意设计的：它是 8 位机到 32 位机的一个平滑过渡，也是为什么从 AVR 或 51 转过来的人几乎不需要适应期。

- **R0~R12**：通用寄存器，放数据、放地址、放参数。函数调用的前四个参数就放在 R0~R3 里，这也是为什么 C 函数参数超过四个之后性能会略微下降——多出来的要压栈。
- **R13 / SP（Stack Pointer）**：栈指针。这是本篇第一个重点，下面单独说。
- **R14 / LR（Link Register）**：链接寄存器。BL 指令跳转前会把返回地址写进 LR。函数里如果再调用别的函数，就得先把 LR 压栈，否则返回地址就丢了——这是递归和深层调用栈溢出的一个源头。
- **R15 / PC（Program Counter）**：程序计数器。注意 Cortex-M 是 Thumb 指令集，PC 的最低位永远是 0，写地址到 PC 做跳转时最低位要置 1 表示 Thumb 状态。Bootloader 里那句经典写法 `((void(*)(void))0x08004000)` ——实际跳转地址会变成 0x08004001——就是 Thumb 位的产物。
- **xPSR（Program Status Register）**：程序状态寄存器，实际由 APSR（算术标志）、IPSR（当前异常号）、EPSR 三部分组成。IPSR 在调试时很有用：看一眼它就知道当前停在哪个中断里。

### 为什么要有两个栈指针

Cortex-M 有两个物理栈指针：**MSP（主栈指针）**和 **PSP（进程栈指针）**。任意时刻只有一个是"当前的"，由 CONTROL 寄存器的 SPSEL 位决定。

这个设计对裸机开发者几乎透明，但对 RTOS 是决定性的：

**第一，内核和任务用不同的栈。** 异常处理（Handler 模式）**强制**使用 MSP，应用代码（Thread 模式）可以用 PSP。这样一来，某个任务把栈写爆了，最多毁掉自己的 PSP，不会连着内核栈一起冲掉——否则一个任务的栈溢出会让整个系统莫名其妙地死掉，而且极难定位。

**第二，任务切换变得极简。** RTOS 切换任务时不需要搬移栈内容，只要把 PSP 改成新任务的栈顶，再恢复它的寄存器现场就行。如果没有 PSP，每次切换都得把整个栈拷贝一遍，代价高得离谱。

**第三，可以给每个任务分配独立的栈。** 你才能回答"这个任务到底需要多大栈"这个问题。裸机只有一个栈，栈多大全凭猜；有了 PSP，每个任务的栈可以单独测量和调整。

所以如果你以后要学 RTOS（第 4 章），回头再看一遍 PSP 这一段，会突然明白很多设计。
...
这张表里，PendSV 和 SVCall 是专门为 RTOS 留的。SVCall 和 SysTick 又被保留，这个细节值得琢磨：**ARM 在设计内核时就预料到了人们会拿它跑操作系统。**
...
**注意第 1.2 篇讲的内存布局在这里闭环了。** 如果你当时没看懂链接脚本和段的概念，现在回头看会清晰很多；如果现在还是不清晰，说明 1.2 需要重读——这两篇是强绑定的。
...
**为什么说它和 RTOS 关系密切？** 因为 RTOS 的整个调度逻辑都建立在"节拍"上：每个 tick 中断里，内核检查有没有任务需要唤醒、有没有任务的时间片用完、有没有延时到期。SysTick 中断的优先级通常被设成最低（0xFF），这样它不会打断其他中断——但这也意味着，**如果你的高优先级中断执行时间超过了节拍周期，系统节拍就会被拖慢。**
...
3. **优先掌握这四件事**：向量表前两项的含义、MSP 与 PSP 的分工、优先级数值方向与分组的区别、VTOR 的作用。这四点覆盖了日后 90% 的底层调试场景，其余细节可以现查。
...
**双栈指针是本篇最容易被低估的知识点。** 今天看起来它只是两个寄存器，但它决定了 RTOS 能不能高效工作、决定了一个任务的栈溢出会不会毁掉整个系统、决定了你将来能不能回答"这个任务需要多大栈"。而**启动流程的排查和一次 HardFault 的排查，本质上是同一种能力**——都是在没有源码、只有硬件行为的层次上推理。把"上电到 main"这条链条走通一次，你就具备了这个层次的思维，而这正是从"会改例程"到"能定位问题"的分界线。
