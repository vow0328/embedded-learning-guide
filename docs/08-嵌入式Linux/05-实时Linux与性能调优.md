...
启动时间在很多产品里是硬指标（车载、消费电子、工业设备都可能有要求）。**具体手段在 [8.4 系统移植与构建](04-系统移植与构建.md) 里已经列过**——精简服务、并行启动、延迟加载、裁掉内核里不用的功能、缩短内核的固定等待，这里不重复。

这一节只补上 8.4 没覆盖的那一半：**启动时间也要按最坏值衡量，而不是看最快的那一次。** 冷启动和热启动、存储介质快慢、是否需要做文件系统检查、有没有服务在超时重试，都会让同一份固件的启动时间差出好几秒。所以量启动时间要量几十次、看分布，而不是测一次取个好看的数字。**启动时间从几十秒压到几秒是常见且现实的**，但"压到了几秒"这个结论，只有在你量的是最坏情况时才站得住。
...
延迟的五大来源里，内核不可抢占区和缺页异常是最容易被忽略、也最影响最坏值的两个。前者需要内核改造才能解决，后者只要做好内存锁定就能消除一大半。这也说明了一个规律：**实时性问题往往不在于算法快不快，而在于系统里有多少"不可预测的停顿"。**
